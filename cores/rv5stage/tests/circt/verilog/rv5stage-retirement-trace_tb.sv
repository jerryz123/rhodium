// Drives exact next-PC predictions, squash/replay/traps, and delayed WRS/CMO retirement through public core ports.
// SPDX-License-Identifier: Apache-2.0
module rv5stage_retirement_trace_tb;
  import "DPI-C" function void retirement_trace_init();
  import "DPI-C" function void retirement_trace_expect(longint unsigned pc, int unsigned instruction, int unsigned prediction);
  ...
  task automatic packet(input longint unsigned address, input logic [31:0] word,
                        input bit prediction=0, input longint unsigned destination=0);
    int waited;
    pc=address; instruction=word; predicted=prediction; target=destination; packet_valid=1;
    waited=0; #1;
  ...
  task automatic retire(input longint unsigned address, input logic [31:0] word,
    ...
    retirement_trace_expect(address,word,status);
    packet(address,word,prediction,destination); idle(); retirement_trace_pending(0);
  endtask
  ...
  initial begin
    ...
    retire('h50,32'h00008067,1,1,'h80); // return target right, RAS action None is wrong
    retire('h50,32'h00008067,2,1,'h84); // JALR target miss
    retire('h60,32'h0000a001,1,1,'h60); // C.J self, raw compressed capture
    retire('h60,32'h0000a001,2); // compressed fallthrough is PC+2
    retirement_trace_expect('h70,32'h00000463,2);
    ...
    data_ready=0; packet('h80,32'h00003203); idle(); retirement_trace_pending(0);
    ...
    packet('h94,32'hfff022f3); idle(); retirement_trace_pending(0); // nonexistent CSR
    ...
    reservation=1; retirement_trace_expect('ha0,32'h00d00073,0);
    packet('ha0,32'h00d00073); idle(12); retirement_trace_pending(1);
    reservation=0; idle(); retirement_trace_pending(0);
    // Accepted CMO retires on completion, not on request admission.
    retirement_trace_expect('hb0,32'h0010200f,0); // cbo.clean (x0)
    packet('hb0,32'h0010200f); idle(12); retirement_trace_pending(1);
