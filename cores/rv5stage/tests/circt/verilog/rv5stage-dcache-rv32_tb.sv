...
module rv5stage_dcache_rv32_tb;
  ...
  task automatic send_request(input logic [31:0] address, input logic [3:0] access,
    ...
    core_in.request.bits.byte_mask = 4'(((1 << (1 << size)) - 1) << address[1:0]);
  ...
  initial begin
    ...
    expect_response(0);
    repeat (160) tick();
    assert (core_out.reservation_valid) else $fatal(1, "RV32 protection expiry discarded an unrevoked reservation");
    send_request(32'h103c, 4'd4, 0, 2, 0, 32'h5678);
    expect_response(0);
    ...
    begin
      ...
      for(int lane=0;lane<4;lane++) begin
        ...
        pipeline_in.request='{valid:1,bits:'{byte_mask:4'(((1 << (1 << (2'd0))) - 1) << ((32'h103c+32'(lane)) % 4)),address:32'h103c+32'(lane),access:4'd2,width:2'd0,unsigned_0:0,data:32'ha0+32'(lane)}};
        #1;
