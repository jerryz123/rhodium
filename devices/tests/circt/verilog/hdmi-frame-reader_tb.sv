// Verifies nonallocating CHI frame reads, retry routing, ordering, row markers, and failures.
// SPDX-License-Identifier: Apache-2.0
module hdmi_frame_reader_tb;
  ...
  logic reset = 1'b1;
  identity_t identity;
  struct packed { logic valid; HDMIFrameReaderCommand bits; } command_in;
  ready_t command_out;
  ready_t line_in;
  struct packed { logic valid; HDMIFrameReaderLine bits; } line_out;
  chi_in_t chi_in;
  chi_out_t chi_out;
  logic active;
  logic done;
  ...
  task automatic start_frame(input logic [43:0] address);
    begin
      command_in = '0;
      command_in.valid = 1'b1;
      command_in.bits.base_address = address;
      command_in.bits.pas = 3'h3;
      command_in.bits.qos = 4'ha;
      #1;
      assert(command_out.ready) else $fatal(1, "HDMI frame reader rejected its command");
      tick();
      command_in = '0;
    end
  endtask
  ...
  task automatic accept_request(input logic [43:0] address,
    ...
    begin
      ...
      assert(chi_out.req.bits.address == address &&
        ...
        else $fatal(1, "HDMI frame reader issued an incorrect ReadOnce request");
  ...
  task automatic send_response(input logic [11:0] txn_id,
    ...
    begin
      ...
      assert(chi_out.rsp.response.ready)
        else $fatal(1, "HDMI frame reader did not route a response");
      tick();
      chi_in.rsp.response = '0;
    end
  endtask
    ...
    begin
      for (int packet = 0; packet < 4; packet++) begin
        ...
        assert(chi_out.dat.response.ready)
          else $fatal(1, "HDMI frame reader did not route response data");
        tick();
        chi_in.dat.response = '0;
      end
    end
  ...
  task automatic accept_line(input logic [7:0] base,
                             input bit row_end,
                             input bit frame_end,
                             input logic [1:0] error = 0,
                             input bit poisoned = 0);
    HDMIFrameReaderLine held;
    begin
      while (!line_out.valid) tick();
      held = line_out.bits;
      repeat (2) begin
        tick();
        assert(line_out.valid && line_out.bits == held)
          else $fatal(1, "stalled HDMI frame line changed");
      end
      assert(line_out.bits.data == line_payload(base) &&
             line_out.bits.row_end == row_end &&
             line_out.bits.frame_end == frame_end &&
             line_out.bits.error == error &&
             line_out.bits.poisoned == poisoned)
        else $fatal(1, "HDMI frame reader returned an incorrect line");
      line_in.ready = 1'b1;
      #1;
      assert(done == frame_end) else $fatal(1, "HDMI frame reader final marker mismatch");
      tick();
      line_in = '0;
      assert(!done) else $fatal(1, "HDMI frame reader held done beyond the final transfer");
    end
  endtask

  initial begin
    identity = '{node_id: NODE_ID, home_id: HOME_ID};
    command_in = '0;
    line_in = '0;
    chi_in = '0;
    ...
    start_frame(44'h1000);
    assert(active && !fetch_failed) else $fatal(1, "HDMI frame reader did not start cleanly");
    ...
    line_in.ready = 1'b1;
    tick();
    assert(underflow && starved) else $fatal(1, "HDMI frame reader did not expose starvation");
    line_in = '0;
    ...
    accept_line(8'h10, 1'b0, 1'b0);
    accept_request(44'h10c0, 12'd16);
    accept_line(8'h20, 1'b1, 1'b0, 2'b10, 1'b1);
    assert(fetch_failed) else $fatal(1, "HDMI frame reader did not retain fetch failure");
    accept_line(8'h30, 1'b0, 1'b0);
    send_line(12'd16, 12'h126, 8'h40);
    while (acknowledgements != 4) tick();
    accept_line(8'h40, 1'b1, 1'b1);
    assert(!active && starved && fetch_failed)
      else $fatal(1, "HDMI frame reader did not retire with sticky status");
    assert(!chi_out.dat.request.valid)
      else $fatal(1, "read-only HDMI frame reader emitted request data");

    start_frame(44'h2000);
    assert(active && !fetch_failed) else $fatal(1, "new HDMI frame did not clear failure status");
    accept_request(44'h2000, 12'd17);

    $display("HDMI frame reader retry, ordering, row markers, and status passed");
    $finish;
  end

  initial begin
    #30000;
    $fatal(1, "HDMI frame reader timeout");
  end
endmodule
