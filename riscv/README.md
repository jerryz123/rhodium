...
8/16/32/64-bit EEW. Ordered and unordered indexed segment
`vluxseg*`/`vloxseg*`/`vsuxseg*`/`vsoxseg*` forms cover every NFIELDS value
and 8/16/32/64-bit index EEW. Fixed-unmasked `vlm.v`/`vsm.v` mask transfers
use byte-granular `vstart` and an effective length of `ceil(vl/8)`.
Unit-stride fault-only-first loads include
`vle8/16/32/64ff.v` and every `vlseg2-8e8/16/32/64ff.v` form.
Whole-register transfers include every `vl1/2/4/8re8/16/32/64.v` load and
`vs1/2/4/8r.v` store.
`VectorUnitStrideMemoryInstructions`, `VectorStridedMemoryInstructions`, and
the ordered/unordered indexed, unit-stride segment, constant-stride segment,
ordered/unordered indexed segment, mask, fault-only-first, and whole-register
...
`VectorFixedPointShiftInstructions` contains `vssrl` and `vssra` in `.vv`,
`.vx`, and `.vi` forms. `VectorNarrowingClipInstructions` contains `vnclipu`
and `vnclip` in `.wv`, `.wx`, and `.wi` forms.
`VectorSaturatingAddSubtractInstructions` contains `vsaddu`, `vsadd`, `vssubu`,
...
`VectorExtensionInstructions` contains `vzext.vf2/vf4/vf8` and
`vsext.vf2/vf4/vf8`, with a unary vector format that exposes only `vd`, `vs2`,
and `vm` while retaining each selector as an encoding constraint.
...
`vremu`; `VectorMultiplyInstructions` contains `vmul`, `vmulh`, `vmulhu`, and
`vmulhsu`; and `VectorFractionalMultiplyInstructions` contains fixed-point
`vsmul`. `VectorWideningMultiplyInstructions` contains `vwmulu`, `vwmulsu`, and
`vwmul`. `VectorMultiplyAccumulateInstructions` contains `vmacc`, `vnmsac`,
`vmadd`, and `vnmsub`; `VectorWideningMultiplyAccumulateInstructions` contains
`vwmaccu`, `vwmacc`, `vwmaccsu`, and the scalar-only `vwmaccus.vx`. Other
operations have `.vv` and `.vx` forms using the canonical OPMVV/OPMVX
encodings. `VectorMultiplyDivideInstructions` composes all six catalogs;
result shaping, accumulation, rounding, and sticky saturation remain core policy.
`VectorMoveMergeInstructions` contains `vmv.v.v/v.x/v.i` and
...
`VectorWholeRegisterMoveInstructions` contains `vmv1r.v`, `vmv2r.v`,
`vmv4r.v`, and `vmv8r.v`. Its format exposes only `vd` and `vs2`; the
fixed-unmasked bit and encoded register count remain instruction constraints.
...
combines the eight same-width sum, logic, and signed/unsigned min/max reductions
with `vwredsumu.vs` and `vwredsum.vs`; the same-width and widening catalogs are
...
`VectorGatherInstructions` contains `vrgather.vv/vx/vi` and `vrgatherei16.vv`;
immediate indices use the unsigned five-bit format.
`VectorCompressInstructions` contains the fixed-unmasked `vcompress.vm` form;
its mask/data/destination overlap and restart restrictions remain core policy.
...
| [`isa/zihintntl.rhm`](isa/zihintntl.rhm) | `Zihintntl`, `ZihintntlCompressed` | Zihintntl 1.0 operand-free NTL.P1/PALL/S1/ALL and named C.NTL aliases |
...
The locality-hint catalogs follow [Zihintntl 1.0](https://docs.riscv.org/reference/isa/v20260120/unpriv/zihintntl.html).
Their exact encodings overlap ADD and C.ADD. `ZihintntlCompressed` is a separate
alias catalog, not an extension to append to the disjoint C/Zca catalogs:
existing C/Zca expansion already produces the same canonical 32-bit words.
The encoded register selectors are fixed hint fields, not explicit operands.
...
It describes the instructions, not reservation tracking, waiting, interrupt
wakeup, or `mstatus.TW` timeout policy.
