...
Every Racket or Rhombus invocation must use the repository wrappers and their
persistent worktree-specific `PLTCOMPILEDROOTS`; direct `racket` invocations
outside the wrapper must use `-y`. Generated bytecode, solver logs, and
counterexample artifacts remain outside version control.
...
7. Run the focused formal target and package boundaries through the persistent
   worktree-specific `PLTCOMPILEDROOTS` wrapper.
