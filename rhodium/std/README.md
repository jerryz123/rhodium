...
| Ready-valid protocol declarations | [`ready-valid.rhdl`](ready-valid.rhdl) | `ValidCtrl`, `Valid`, `Decoupled`, `Irrevocable`, and transfer detection |
...
| Interface | Contract |
|---|---|
| `ValidCtrl()` | Every asserted cycle is one payloadless control event; no backpressure |
...
`Valid(T)` refines `ValidCtrl()`. `Decoupled` refines `DecoupledCtrl`; `Irrevocable` refines
