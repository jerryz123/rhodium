...
module event_vector_tb;
  ...
  endtask
  task automatic run_back_to_back_compute(input logic [31:0] insn, input int length);
    int launches;
    bit finished;
    instruction=insn; vl=64'(length); request_valid=1;
    slow=0; launches=0; finished=0;
    disposition=0; issue_ready=1; response_valid=0;
    for(int step=0;step<600 && !finished;++step) begin
      issue_ready=step%7!=2 && step%7!=3;
      responses(); tick();
      if(sampled_launch) begin
        ++launches;
        if(launches==2) request_valid=0;
      end
      if(launches==2 && !active) finished=1;
    end
    assert(finished) else $fatal(1,"back-to-back vector macros did not drain");
    response_valid=0; issue_ready=1;
    repeat(4) tick();
  endtask
