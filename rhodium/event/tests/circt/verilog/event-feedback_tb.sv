// Drives singleton and paired recirculation and compares both against an untraced lane.
...
module event_feedback_tb;
  ...
  forward_t source, sink[3];
  reverse_t source_ready[4], sink_ready;
  EventFeedback dut(.clock(clock), .reset(reset), .select_feedback(select_feedback), .route_feedback(route_feedback),
    ...
    .ingress_2_in(source), .ingress_2_out(source_ready[2]),
    .ingress_3_in(source), .ingress_3_out(source_ready[3]),
    .egress_0_out(sink[0]), .egress_0_in(sink_ready),
    .egress_1_out(sink[1]), .egress_1_in(sink_ready),
    .egress_2_out(sink[2]), .egress_2_in(sink_ready));
  ...
  initial begin
    ...
    for(int step=0; step<330; ++step) begin
      ...
      if(!reset) begin
        ...
        assert((!source.valid || (source_ready[2]==source_ready[1] && source_ready[3]==source_ready[1])) && sink[2].valid==sink[1].valid && (!sink[2].valid || sink[2].bits==sink[1].bits))
          else $fatal(1,"paired lineage changed public behavior");
      end
