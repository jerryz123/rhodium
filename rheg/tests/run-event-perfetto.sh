...
for suffix in '' .gz; do
  ...
  assert_query "$file" "SELECT count(*)=9 AND count(DISTINCT s.track_id)=7 AND sum(s.name='stall' AND s.dur=20 AND t.name='chi.rxdat')=1 AND sum(s.name='c')=2 AND sum(s.name='trailing.')=1 AND sum(t.name='physical/instance/event:7')=1 AS ok FROM slice s JOIN track t ON t.id=s.track_id"
  assert_query "$file" "SELECT count(*)=1 AS ok FROM track WHERE name='chi.rxdat'"
...
assert_query "$stream_test_dir/build/enums.pftrace" "WITH expected(track,seq,value) AS (VALUES('cache.txreq','0','ReadShared'),('cache.txreq','1','ReadClean'),('cache.txreq','2','0x7f'),('cache.txrsp','0','SnpResp'),('cache.txrsp','1','CompAck'),('cache.txrsp','2','0x7f'),('unselected','0','NotALabel'),('unselected','1','0x02'),('unselected','2','0x7f'),('wide','0','All'),('wide','1','All'),('wide','2','All')) SELECT count(*)=12 AND sum(s.dur=10 AND s.ts=10*CAST(e.seq AS INT))=12 AND sum(s.name=CASE t.name WHEN 'unselected' THEN 'unselected' ELSE e.value END)=12 AND sum(EXTRACT_ARG(s.arg_set_id,CASE t.name WHEN 'cache.txrsp' THEN 'debug.operation' ELSE 'debug.opcode' END)=e.value)=12 AS ok FROM slice s JOIN track t ON t.id=s.track_id JOIN expected e ON e.track=t.name AND e.seq=EXTRACT_ARG(s.arg_set_id,'debug.sequence')"
assert_query "$stream_test_dir/build/enums.pftrace" "SELECT count(*)=12 AND sum(value_type='string')=12 AS ok FROM slice s JOIN args a USING(arg_set_id) WHERE a.key IN ('debug.opcode','debug.operation')"
...
assert_query "$stream_test_dir/build/enums.pftrace.override" "SELECT count(*)=1 AND min(s.name)='Explicit' AND min(t.name)='decode' AND min(EXTRACT_ARG(s.arg_set_id,'debug.opcode'))='li a0, 5' AND min(EXTRACT_ARG(s.arg_set_id,'debug.instruction'))='Explicit' AS ok FROM slice s JOIN track t ON t.id=s.track_id"
...
assert_query "$stream_test_dir/build/repeated-label.pftrace" "SELECT count(*)=3 AND count(DISTINCT track_id)=1 AND sum(name='accepted')=3 AS ok FROM slice"
