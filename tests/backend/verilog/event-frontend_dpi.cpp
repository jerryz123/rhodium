...
extern "C" void event_frontend_sample(unsigned reset, unsigned clear, unsigned restart,
  ...
  // A public instruction offer reads old words; current fills append only at the edge.
  if (fetched_valid) {
    if (clear || words.empty() || (pc & ~3ULL) != words.front().base) fail("frontend word order mismatch");
  ...
  if (clear) {
    ...
    words.clear(); s1.reset(); s2.reset();
    if (restart) cursor = restart_pc;
    next_attempt = cursor;
...
extern "C" void event_frontend_finish() {
  rheg::graph().validate();
  if (!shared || !straddles || !faults || !stalls || !canceled) fail("frontend trace coverage incomplete");
}
