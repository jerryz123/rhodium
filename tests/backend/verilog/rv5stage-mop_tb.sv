...
module rv5stage_mop_tb;
  ...
  function automatic logic [31:0] instruction_at(input logic [63:0] address);
    case (address)
      64'h00000001_00000000: instruction_at = 32'h00700413; // addi x8, x0, 7
      64'h00000001_00000004: instruction_at = 32'h81cfc473; // mop.r.0 x8, x31
      64'h00000001_00000008: instruction_at = 32'h00900093; // addi x1, x0, 9
      64'h00000001_0000000c: instruction_at = 32'h00016081; // c.mop.1; c.nop
      64'h00000001_00000010: instruction_at = 32'h00140533; // add x10, x8, x1
      64'h00000001_00000014: instruction_at = 32'h00a00023; // sb x10, 0(x0)
      default: instruction_at = 32'h00000013; // nop
    endcase
  endfunction
  ...
  always_ff @(posedge clock) begin
    ...
    if (!reset && data_access_out.request.valid && data_access_in.request.ready) begin
      assert (data_access_out.request.bits.access == MEMORY_STORE &&
        ...
        else $fatal(1, "MOP architectural write behavior was incorrect");
      assert (!fault) else $fatal(1, "MOP program raised an unexpected fault");
      $display("RV5Stage Zimop and Zcmop execution passed");
      $finish;
    end
  end
  ...
  initial begin
    ...
    $fatal(1, "MOP program did not reach its byte store");
  end
endmodule
