// Checks boot programming after host drain, CHI stalls/errors, release, RV32 limits, and reset.
module fesvr_boot_tb;
  ...
  endtask
  task automatic write_transaction(logic [63:0] value, int error_kind = 0);
    wait (port_out.requests.valid); #1;
    repeat (4) begin
      assert(port_out.requests.valid && port_out.requests.bits.address == 44'h3000);
    ...
    stalled(); // Accepting DAT is not sufficient to release the cores.
    port_in.responses.valid = 1;
  ...
  initial begin
    ...
    requests_in = '{valid: 1, bits: '{write: 1, address: 64'h3000, data: 64'hdeadbeef, length: 8'd8}};
    entry_in = '{valid: 1, bits: 64'h80002000};
    tick(); requests_in.valid = 0;
    write_transaction(64'hdeadbeef);
    repeat (4) begin
