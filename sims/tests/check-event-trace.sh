...
assert_query "SELECT count(*)>0 AND sum(t.name='memory-request')>0 AND sum(t.name='memory-accept')>0 AND sum(t.name NOT IN ('memory-request','memory-accept','memory-response','soc-response','core.s1.fetch','core.s2.decode','core.s3.execute','core.s4.memory','core.s5.wb','mmu.pte.request') AND t.name NOT GLOB '[id]cache.*' AND t.name NOT GLOB 'frontend.*')=0 AND sum(s.dur<10 OR s.dur%10!=0)=0 AND max(s.depth)=0 AS ok FROM slice s JOIN track t ON t.id=s.track_id"
