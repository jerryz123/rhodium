...
assert_query "SELECT count(*)>0 AND count(*)*2=(SELECT count(*) FROM slice s JOIN track t ON t.id=s.track_id WHERE t.name NOT GLOB 'core.*') AND sum(a.ts!=b.ts)=0 AND sum((a.name||'->'||b.name) NOT IN ('memory-request->memory-accept','memory-response->soc-response'))=0 AS ok FROM flow JOIN slice a ON a.id=flow.slice_out JOIN slice b ON b.id=flow.slice_in JOIN track t ON t.id=a.track_id WHERE t.name NOT GLOB 'core.*'"
...
assert_query "SELECT count(*)=0 AS ok FROM slice s JOIN args a USING(arg_set_id) WHERE a.key NOT IN ('debug.sequence','debug.cycle','debug.pc','debug.instruction','debug.raw')"
