...
assert_query "SELECT count(*)>0 AND sum(t.name='memory-request')>0 AND sum(t.name='memory-accept')>0 AND sum(t.name NOT IN ('memory-request','memory-accept','memory-response','soc-response','core.s2.decode','core.s3.execute','core.s4.memory','core.s5.wb','mmu.pte.request','home.request','home.response','home.data') AND t.name NOT GLOB '[id]cache.*' AND t.name NOT GLOB 'frontend.*')=0 AND sum(s.dur<10 OR s.dur%10!=0)=0 AND max(s.depth)=0 AS ok FROM slice s JOIN track t ON t.id=s.track_id"
assert_query "SELECT count(*)>0 AND sum(s.name=CASE t.name WHEN 'frontend.s0.request' THEN 'request' WHEN 'frontend.s1.lookup' THEN 'lookup' WHEN 'frontend.s2.outcome' THEN 'outcome' WHEN 'mmu.pte.request' THEN 'request' ELSE t.name END AND s.dur=10)=count(*) AS ok FROM slice s JOIN track t ON t.id=s.track_id WHERE t.name NOT GLOB 'core.*' AND t.name NOT GLOB '[id]cache.*' AND t.name NOT GLOB 'home.*'"
...
assert_query "SELECT count(*)=4 AND sum(json_extract(a.string_value,'$.fields[1].encoding')='riscv' AND json_extract(a.string_value,'$.fields[1].isa') GLOB 'rv64i*' AND json_extract(a.string_value,'$.fields[1].pc')='pc')=4 AS ok FROM track t JOIN args a ON a.arg_set_id=t.source_arg_set_id WHERE t.name GLOB 'core.*' AND a.key='description' AND json_extract(a.string_value,'$.kind')='transfer'"
