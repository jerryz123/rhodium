...
SELECT
  ...
  (SELECT count(*)>0 FROM edges WHERE src='dcache.s4.resolve' AND dst='dcache.txreq') AND
  (SELECT count(*)=0 FROM edges WHERE dst='dcache.txreq' AND
  ...
  (SELECT count(*)=0 FROM events e WHERE name='dcache.txreq' AND
  ...
  (SELECT count(*)=0 FROM events e WHERE name='dcache.s4.resolve' AND EXTRACT_ARG(arg_set_id,'debug.refill_accepted')=1 AND
    NOT EXISTS (SELECT 1 FROM edges WHERE parent=e.id AND dst='dcache.txreq')) AND
