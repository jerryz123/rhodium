-- Checks selected fetch causes, stage timing, and retained/live S2-to-instruction ancestry.
...
SELECT
  ...
  (SELECT count(*)>0 FROM edges WHERE src IN ('core/s4.memory','core/s5.wb') AND dst='frontend/s0.request') AND
  (SELECT count(*)=0 FROM edges WHERE
    (src||'->'||dst) NOT IN ('frontend/s0.request->frontend/s1.lookup',
      ...
      'core/s4.memory->frontend/s0.request','core/s5.wb->frontend/s0.request') OR
