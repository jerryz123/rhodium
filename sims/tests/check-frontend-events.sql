-- Checks fetch-attempt stages and retained S2-to-instruction ancestry in the native importer.
WITH events AS MATERIALIZED (
  ...
  WHERE (t.name GLOB 'frontend.*' OR t.name='core.s1.fetch') AND s.name!='stall'
...
SELECT
  ...
  (SELECT count(*)=0 FROM edges WHERE
    (src||'->'||dst) NOT IN ('frontend.s0.request->frontend.s1.lookup',
      'frontend.s1.lookup->frontend.s2.outcome','frontend.s2.outcome->core.s1.fetch') OR
    (src IN ('frontend.s0.request','frontend.s1.lookup') AND (delay!=10 OR parent_pc!=child_pc)) OR
    (src='frontend.s2.outcome' AND (delay<10 OR replay IS NOT 0 OR admitted IS NOT 1))) AND
  ...
  (SELECT count(*)=0 FROM events e WHERE name='core.s1.fetch'
