-- Checks private-cache CHI schemas, Home returns, and line-completing CompAck ancestry.
...
WITH expected(suffix, fields) AS (
  ...
  SELECT t.id, t.name, a.string_value AS schema,
         ...
         t.name AS channel
  FROM rheg_tracks t JOIN args a ON a.arg_set_id=t.source_arg_set_id
  WHERE t.name GLOB '[id]cache/*' AND substr(t.name,12) IN (SELECT suffix FROM expected) AND a.key='description'
...
SELECT
  ...
  (SELECT count(*)=(SELECT count(*) FROM cache_tracks) FROM cache_tracks t JOIN expected e ON substr(t.channel,12)=e.suffix
  ...
  (SELECT count(*)>0 FROM events WHERE channel='icache/chi.txreq' AND kind='transfer') AND
  (SELECT count(*)>0 FROM events WHERE channel='icache/chi.rxdat' AND kind='transfer') AND
  (SELECT count(*)>0 FROM events WHERE channel='dcache/chi.txreq' AND kind='transfer') AND
  (SELECT count(*)>0 FROM events WHERE channel='dcache/chi.rxdat' AND kind='transfer') AND
  ...
  (SELECT count(*)=0 FROM events
   WHERE (channel GLOB '*.tx*' AND EXTRACT_ARG(arg_set_id,'debug.src_id')!=CASE substr(channel,1,6) WHEN 'icache' THEN 2 ELSE 3 END)
      OR (channel IN ('icache/chi.rxrsp','icache/chi.rxdat','dcache/chi.rxrsp','dcache/chi.rxdat')
          AND EXTRACT_ARG(arg_set_id,'debug.tgt_id')!=CASE substr(channel,1,6) WHEN 'icache' THEN 2 ELSE 3 END)) AND
  (SELECT count(*)>0 FROM events
   WHERE channel='icache/chi.txreq' AND EXTRACT_ARG(arg_set_id,'debug.address')=printf('0x%011x',2147483648)
  ...
  (SELECT count(*)=0 FROM flow f JOIN events p ON p.id=f.slice_out
   ...
   WHERE p.kind!='transfer' OR NOT (
     (p.channel IN ('icache/chi.txreq','dcache/chi.txreq') AND
       t.name IN (substr(p.channel,1,11)||'rxrsp',substr(p.channel,1,11)||'rxdat')) OR
     (p.channel IN ('icache/chi.rxdat','dcache/chi.rxdat') AND
       t.name=substr(p.channel,1,11)||'txrsp' AND c.name='CompAck'))) AND
