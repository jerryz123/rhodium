...
FROM track
WHERE json_extract(EXTRACT_ARG(source_arg_set_id,'description'),'$.kind') IN ('transfer','residency')
   OR json_type(EXTRACT_ARG(source_arg_set_id,'description'),'$.sites')='array';
