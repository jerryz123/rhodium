...
SELECT
  (SELECT count(*)>0 FROM edges WHERE src='dcache.txreq' AND dst='dcache.rxdat') AND
  -- Opaque branches may lack parents, but must explicitly report that gap.
  (SELECT count(*)=0 FROM events e WHERE name IN ('dcache.rxrsp','dcache.rxdat')
  ...
  (SELECT count(*)=0 FROM edges WHERE
    (dst IN ('dcache.rxrsp','dcache.rxdat') AND
      (src!='dcache.txreq' OR delay<10 OR
